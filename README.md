...
- **A file-system-aware / CLI AI** (e.g. Claude Code) — gives a second, *habit-aware* opinion on the code specifically, and **logs your session report** into the `logs/` files.
...
7. **(Recommended) Also paste the code into a file-system-aware / CLI AI** (e.g. Claude Code) for a second, *habit-aware* opinion on the code specifically.
8. **Debrief** — discuss, diagnose, and generate the session report ([`session-report-template.md`](session-report-template.md)).
9. **Paste the report into the file-system-aware AI to log it** into your `logs/` files, and set the next-attempt date in [`logs/spaced-rep-queue.md`](logs/spaced-rep-queue.md).
